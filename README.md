...
# :loudspeaker:Further Study on Improving Neural Networks
## 신경망 성능 향상을 위한 정규화 기법
- 심층 신경망과 같은 복잡한 모델은 보이지 않는 데이터로 일반화하는 대신 훈련 세트의 특이한 패턴을 기억하는 유연성 때문에 과대적합되기 쉽다.
==>해결방법 : 정규화
- 정규화: 훈련 오류가 아닌 일반화 오류를 줄이기 위해 학습 알고리즘을 수정하는 것
---------------------------------------------------------------------------
### 1. 높은 편향과 높은 분산이 테스트 오류에 미치는 영향
- 기계 학습/딥 러닝에서 목표: 입력과 출력 간의 관계를 근사화하는 것

- 편향: 가능한 모든 훈련 세트에 맞춰진 모든 모델의 평균이 실제 모집단 선에서 얼마나 멀리 떨어져 있는지 정량화하는것

- 분산:추정치가 평균을 중심으로 얼마나 다른지를 측정. 분산은 예측에 사용하는 모델이 모든 훈련 데이터 세트에 대해 예상되는 적합성에서 얼마나 벗어나는가를 측정.

==>특징:
- 단순 모델: 높은 편향, 낮은 분산
- 복잡한 모델: 낮은 편향, 높은 분산
- 편향과 분산은 모두 평균 제곱 오차 손실에 기여하기 때문에 항상 절충 관계에 있습니다.

==>분석:
==> 단순 모델을 예측에 사용하면 단순 모델의 편향이 높기 때문에 예측 값이 실제 출력과 멀어집니다.(과소적합)

==>반면 복잡한 모델을 사용하는 경우 보이지 않는 데이터를 사용하여 예측한 값은 훈련 데이터에서 얻은 값에 가깝지 않습니다. 복잡한 모델이 학습 데이터의 패턴을 기억하는 데 너무 집중했기 때문입니다.(과적합)

==>문제:모델 복잡성이 증가함에 따라 학습 오류가 감소하지만 복잡한 모델은 과적합으로 인해 높은 분산을 갖게 됨.  간단한 모델을 사용하면 바이어스가 높기 때문에 훈련 오류와 테스트 오류가 모두 높아짐

==>최적의 모델:우리는 학습 및 테스트 오류가 최소가 되도록 편향과 분산의 균형을 이루는 모델
--------------------------------------------------------------------
### 2.정규화로 문제해결

- 심층 신경망은 매우 복잡한 모델(많은 매개변수와 많은 비선형성)이며 과대적합하기 쉽습니다(훈련 오류를 0으로 유도하기때문).

==>따라서 어떤 형태의 정규화가 필요함
==>다양한 정규화 기술을 사용하여 낮은 훈련 및 테스트 오류를 달성하여 편향과 분산을 완벽하게 교환가능

#### 정규화 기술
- L2 정규화
- 데이터세트 증강
- 조기 중지
- : 정규화 기술은 목적 함수에 파라미터 놈 페널티 Ω(θ)를 추가하여 신경망, 선형 회귀 또는 로지스틱 회귀와 같은 모델의 용량을 제한함으로써 작동


#### 1) L2 정규화

- 정규화된 손실 함수를 최소화하기 위해 알고리즘은 원래 손실 함수와 가중치의 제곱에 따라 달라지는 정규화 항을 모두 최소화해야 합니다. 사실상 네트워크의 가중치가 너무 커지지 않도록 원래 손실 함수에 제약 조건을 추가하는 것.
==>정규화된 용어를 추가함으로써 훈련 오류를 0으로 유도하지 않도록 모델을 속이고 있으며, 이는 모델의 복잡성을 줄임
==> 따라서 L2 정규화는 데이터의 과적합을 줄이는 데 도움됨

 

#### 2)데이터세트 증강

- 데이터 세트 확대는 회전, 뒤집기, 원본 이미지의 일부 픽셀에 블러 추가 또는 변환과 같은 사소한 변경을 수행하여 기존 교육 데이터에서 인위적으로 데이터를 생성

==>더 많은 데이터로 보강하면 신경망이 훈련 오류를 0으로 만드는 것이 더 어려워짐

 

#### 3)조기중지

- 훈련 데이터에 신경망을 맞출 때 각 반복 후 보이지 않는 데이터에 대해 모델을 평가함

- 유효성 검사 데이터에 대한 모델의 성능이 개선되지 않는 경우, 즉... 특정 반복에 대해 유효성 검사 오류가 증가하거나 동일하게 유지되면 모델을 추가로 교육할 필요가 없음
